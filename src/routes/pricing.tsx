...
const TIERS = [
  ...
  },
  {
    ...
    features: [
      ...
      "1 free veneer per year",
      "Cosmetic consultations",
      "30% off all treatments",
      "Concierge SMS line",
      "Free family check-ups",
    ],
  },
];
              ...
              >
                ...
                <ul className="mt-6 space-y-3">
                  {t.features.map((f) => (
                    <li key={f} className="flex items-start gap-2.5 text-sm">
                      <span
                        className={`w-5 h-5 rounded-full grid place-items-center shrink-0 mt-0.5 ${t.popular ? "bg-cream/20" : "bg-primary/10"}`}
                      >
                        <Check
                          className={`w-3 h-3 ${t.popular ? "text-cream" : "text-primary"}`}
                          strokeWidth={3}
                        />
                      </span>
                      {f}
                ...
                <Link
                  ...
                  className={`mt-8 flex items-center justify-center gap-2 font-semibold py-3 rounded-full transition ${
                    t.popular
                      ? "bg-cream text-primary-deep hover:bg-accent-soft"
