...
export const Route = createFileRoute("/dentists/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.dentist.name ?? "Dentist"} at Decare Dental` },
      { name: "description", content: loaderData?.dentist.bio ?? "" },
    ],
  }),
  notFoundComponent: () => (
    <div className="py-32 text-center">
      <h1 className="font-display font-bold text-3xl">Dentist not found</h1>
      <Link to="/dentists" className="mt-4 inline-block text-primary font-semibold">← All dentists</Link>
    </div>
  ),
      ...
      <section className="py-16 lg:py-24 bg-cream">
        <div className="max-w-5xl mx-auto px-5 md:px-6 grid lg:grid-cols-5 gap-8">
          <div className="lg:col-span-2">
            <img src={dentist.img} alt={dentist.name} className="w-full aspect-[4/5] object-cover rounded-3xl shadow-elevated" />
          ...
          <div className="lg:col-span-3">
            <h2 className="font-display font-bold text-2xl text-ink">About {dentist.name.split(" ")[1]}</h2>
            ...
            <div className="mt-4 bg-cream-deep rounded-2xl p-4 overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr>
                    <th></th>
                    {HOURS.map((h) => <th key={h} className="px-1 py-1 text-slate-body font-medium">{h}</th>)}
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={day}>
                      ...
                      {slots.map((s, i) => (
                        <td key={i} className="px-1 py-1">
                          <div className={`h-7 rounded ${s ? "bg-success/70" : "bg-cream"}`} title={s ? "Available" : "Booked"} />
                        </td>
                      ))}
                    </tr>
                  ))}
              ...
              <div className="mt-3 flex items-center gap-4 text-xs text-slate-body">
                <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-success/70" /> Available</span>
                <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-cream border border-border" /> Booked</span>
              </div>
            </div>

            <Link to="/booking" className="mt-8 inline-flex items-center gap-2 gradient-cta text-cream font-semibold px-6 py-3 rounded-full shadow-glow hover:-translate-y-0.5 transition">
