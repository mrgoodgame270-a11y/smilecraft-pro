...
export const Route = createFileRoute("/services/$slug")({
  ...
  notFoundComponent: () => (
    <div className="py-32 text-center">
      ...
      <Link to="/services" className="mt-4 inline-block text-primary font-semibold">← All services</Link>
    </div>
  ),
      ...
      <section className="py-16 lg:py-24 bg-cream">
        <div className="max-w-5xl mx-auto px-5 md:px-6 grid lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2">
            ...
            <h3 className="mt-10 font-display font-bold text-xl text-ink">Why patients choose us</h3>
          ...
          <aside className="bg-cream-deep rounded-3xl p-6 shadow-soft h-fit">
            ...
            <div className="font-display font-extrabold text-3xl gradient-text mt-1">{service.price}</div>
            <Link to="/booking" className="mt-5 w-full gradient-cta text-cream font-semibold py-3 rounded-full shadow-glow flex items-center justify-center gap-2 hover:-translate-y-0.5 transition">
              Book Consultation <ArrowRight className="w-4 h-4" />
            </Link>
            <a href={`tel:${PHONE_TEL}`} className="mt-3 w-full border-2 border-border text-ink font-semibold py-3 rounded-full flex items-center justify-center gap-2 hover:border-primary hover:text-primary transition">
        ...
        <div className="max-w-5xl mx-auto px-5 md:px-6">
          ...
          <div className="flex flex-wrap gap-3">
            {SERVICES.filter((s) => s.slug !== service.slug).map((s) => (
              <Link key={s.slug} to="/services/$slug" params={{ slug: s.slug }} className="bg-cream px-4 py-2 rounded-full text-sm font-medium text-ink hover:bg-accent hover:text-cream transition">
                {s.title}
              </Link>
            ))}
          </div>
