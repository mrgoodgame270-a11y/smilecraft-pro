...
export function BookingFlow() {
  ...
  const [data, setData] = useState({ pain: "", service: "", date: "", time: "", name: "", phone: "", email: "" });
  const days = makeDays();
  ...
  if (data.pain === "emergency" && step === 1) {
    return (
      <div className="bg-cream rounded-3xl p-6 md:p-10 shadow-elevated border-2 border-coral text-center">
        ...
        <p className="mt-2 text-slate-body">For dental emergencies, please call our 24/7 line. We will see you today.</p>
        <a href={`tel:${PHONE_TEL}`} className="mt-6 inline-flex items-center gap-2 bg-coral text-cream font-semibold px-7 py-4 rounded-full shadow-glow hover:-translate-y-0.5 transition">
          <Phone className="w-4 h-4" /> Call {PHONE_DISPLAY}
        </a>
        <button onClick={() => setData({ ...data, pain: "" })} className="block mx-auto mt-4 text-sm text-slate-body hover:text-ink">
  ...
  return (
    <div className="bg-cream rounded-3xl p-6 md:p-8 text-ink shadow-elevated">
      <div className="flex items-center gap-2 mb-6">
        {[0, 1, 2, 3, 4].map((i) => (
          <div key={i} className={`h-1.5 flex-1 rounded-full transition ${i <= step ? "bg-accent" : "bg-cloud"}`} />
        ))}
      </div>
        ...
        >
            ...
            <div>
              ...
              <div className="grid grid-cols-5 gap-2 mt-4">
                {PAIN.map((p) => (
                  <button
                    key={p.value}
                    onClick={() => { setData({ ...data, pain: p.value }); next(); }}
                  ...
                  >
                    ...
                    <span className="text-[10px] md:text-[11px] font-semibold text-slate-body text-center leading-tight">{p.label}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-2 gap-2 mt-4">
                {SERVICES.map((s) => (
                  <button
                    key={s.slug}
                    onClick={() => { setData({ ...data, service: s.title }); next(); }}
          ...
          {step === 4 && (
            <div className="space-y-3">
              ...
              <input value={data.name} onChange={(e) => setData({ ...data, name: e.target.value })} placeholder="Full name" className="w-full h-14 px-4 rounded-xl border-2 border-border focus:border-primary outline-none bg-cream" />
              <input value={data.phone} onChange={(e) => setData({ ...data, phone: e.target.value })} placeholder="Phone number" type="tel" className="w-full h-14 px-4 rounded-xl border-2 border-border focus:border-primary outline-none bg-cream" />
              <input value={data.email} onChange={(e) => setData({ ...data, email: e.target.value })} placeholder="Email (optional)" type="email" className="w-full h-14 px-4 rounded-xl border-2 border-border focus:border-primary outline-none bg-cream" />
              <button onClick={next} className="w-full gradient-cta text-cream font-semibold h-14 rounded-xl shadow-glow flex items-center justify-center gap-2">
          ...
          {step === 5 && (
            <div className="text-center py-6">
              ...
              <p className="mt-2 text-sm text-slate-body">{data.service} · {data.date} · {data.time}</p>
              <button onClick={() => { setStep(0); setData({ pain: "", service: "", date: "", time: "", name: "", phone: "", email: "" }); }} className="mt-4 text-primary font-semibold">Book another →</button>
            </div>
          )}
        </motion.div>
      </AnimatePresence>
