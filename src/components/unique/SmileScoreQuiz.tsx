...
const RECOMMEND: Record<string, { service: string; slug: string; copy: string }> = {
  cosmetic: { service: "Cosmetic Dentistry", slug: "cosmetic", copy: "Veneers, professional whitening or bonding will transform your smile in 2 visits." },
  general: { service: "General Dentistry", slug: "general-dentistry", copy: "Start with a comprehensive exam and gentle cleaning. We will build a plan from there." },
  ortho: { service: "Orthodontics", slug: "orthodontics", copy: "Invisalign aligners can straighten your teeth in 6 to 18 months, almost invisibly." },
  implants: { service: "Dental Implants", slug: "implants", copy: "Same day digital implants restore function and confidence in a single visit." },
};
    ...
    <section className="py-20 lg:py-28 bg-cream">
      <div className="max-w-5xl mx-auto px-5 md:px-6">
        <div className="text-center max-w-2xl mx-auto">
          <span className="text-[12px] md:text-[13px] font-semibold tracking-[0.18em] uppercase text-accent">Smile Score Quiz</span>
          ...
          <p className="mt-4 text-slate-body">3 quick questions. We will recommend the right care for you.</p>
        </div>
        ...
        <div className="mt-10 bg-cream-deep rounded-3xl p-6 md:p-10 shadow-soft">
          {!done && (
            <>
              <div className="flex items-center gap-2 mb-6">
                {QUESTIONS.map((_, i) => (
                  <div key={i} className={`h-1.5 flex-1 rounded-full transition ${i <= step ? "bg-accent" : "bg-cream"}`} />
                ))}
              </div>
              <AnimatePresence mode="wait">
                ...
                >
                  <h3 className="font-display font-bold text-xl md:text-2xl text-ink">{QUESTIONS[step].q}</h3>
          ...
          {done && rec && (
            ...
            >
              ...
              <div className="mt-4 font-display font-extrabold text-7xl gradient-text">{Math.min(score, 95)}</div>
              ...
              <div className="mt-6 flex flex-wrap justify-center gap-3">
                <Link to="/services/$slug" params={{ slug: rec.slug }} className="bg-cream border-2 border-ink/10 text-ink font-semibold px-6 py-3 rounded-full hover:border-primary hover:text-primary transition inline-flex items-center gap-2">
                  Learn More <ArrowRight className="w-4 h-4" />
                </Link>
                <Link to="/booking" className="gradient-cta text-cream font-semibold px-6 py-3 rounded-full shadow-glow hover:-translate-y-0.5 transition inline-flex items-center gap-2">
                  Book Consultation <ArrowRight className="w-4 h-4" />
                </Link>
                <button onClick={reset} className="text-slate-body font-semibold px-4 py-3 rounded-full hover:text-ink inline-flex items-center gap-2">
