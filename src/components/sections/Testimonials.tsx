...
const REVIEWS = [
  {
    ...
    img: doc1,
    quote:
      "I was terrified of dentists. The team made me feel completely calm. I actually look forward to visits now. Best decision I have made for my smile.",
  },
  {
    name: "Andre Bello",
    img: doc4,
    quote:
      "Booked online in under a minute. Got SMS reminders. The implant work was painless and looks incredible. These guys run a tight operation.",
  },
  {
    name: "Mei Tanaka",
    img: doc3,
    quote:
      "Brought my 6 year old here scared and crying. She left asking when she could come back. Genuine, gentle, modern dentistry.",
  },
];
    ...
    return (
      <section className="py-16 lg:py-24 bg-lavender">
        <div className="max-w-3xl mx-auto px-5 md:px-6 text-center">
          ...
          <p className="mt-4 font-display text-xl md:text-2xl text-ink leading-relaxed">
            {r.quote}
          </p>
          ...
          <div className="mt-6 flex justify-center gap-2">
            {REVIEWS.map((_, idx) => (
              <button
                key={idx}
                onClick={() => setI(idx)}
                className={`h-1.5 rounded-full transition-all ${idx === i ? "w-8 bg-accent" : "w-2 bg-primary/25"}`}
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-6">
        <div className="flex items-end justify-between flex-wrap gap-4 mb-10">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="w-12 h-12 rounded-full bg-cream shadow-soft grid place-items-center hover:bg-primary hover:text-cream transition"
              aria-label="Previous"
            >
        ...
        <div className="grid md:grid-cols-3 gap-5 md:gap-6">
          {REVIEWS.map((r, idx) => (
            <div
              key={r.name}
              className={`bg-cream rounded-3xl p-6 md:p-7 shadow-soft transition-all ${idx === i ? "scale-[1.02] shadow-elevated" : "opacity-90"}`}
            >
